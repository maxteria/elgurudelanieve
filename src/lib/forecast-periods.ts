...
export function getTodayForecast(hourly: NormalizedHourlyForecast[]): NormalizedHourlyForecast[] {
  const now = new Date();
...
export function getTomorrowForecast(hourly: NormalizedHourlyForecast[]): NormalizedHourlyForecast[] {
  return hourly.filter(h => {
    const d = parseLocalDate(h.time);
    return isTomorrow(d);
  });
...
export function getSevenDayForecast(hourly: NormalizedHourlyForecast[]): DailySummary[] {
  ...
  for (const [dateStr, entries] of grouped) {
    ...
    const totalPrecip = entries.reduce((sum, e) => sum + Math.max(0, e.precipitation), 0);
    const totalSnow = entries.reduce((sum, e) => sum + Math.max(0, e.snowfall), 0);

...
const ZONE_CONFIG: { id: 'village' | 'mid' | 'top'; legacyName: 'Pueblo' | 'Centro' | 'Cumbre'; label: string }[] = [
...
function snowProbability(temp: number, precip: number, freezingLevel: number, altitude: number): number {
...
export function normalizedToLegacy(normalized: NormalizedSnowForecast): WeatherData {
  ...
  const zones: ZoneForecast[] = ZONE_CONFIG.map(cfg => {
    ...
    const hourly: HourlyForecast[] = normalized.hourly.map(h => {
      ...
      const zoneWind = Math.round(h.wind * (1 + 0.08 * ((zone.altitude - baseAlt) / 100)) * 10) / 10;
      const hour = new Date(h.time).getHours();
      const feelsLike = windChill(zoneTemp, zoneWind);
      const snowProb = snowProbability(zoneTemp, h.precipitation, h.freezingLevel, zone.altitude);

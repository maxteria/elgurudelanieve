# Lab UI Enrichment Specification

## Purpose

Enrich the LAB page UI components to display additional weather data points — freezing level, humidity, and precipitation probability — and ensure dynamic updates on period switching. Created as part of the Lab Pro Dashboard change.
...
- WHEN ZoneCard renders the data section
...
- WHEN ZoneCard renders the snow depth row
...
`setActivePeriod()` in `lab.astro` JS MUST query and update `[data-period-zone-snowdepth]`.

#### Scenario: Snow depth updates on period switch

- GIVEN a period tab is clicked
- WHEN `setActivePeriod()` runs and iterates zone cards
